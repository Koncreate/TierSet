...
function isForwardRef(candidate: any): candidate is ForwardRefExoticComponent<any> {
...
function isIcon(candidate: any): candidate is Icon {
  return isForwardRef(candidate) && isNamedExoticComponent(candidate) && !isBase(candidate);
}

const allIcons = Object.entries(Icons).filter(([name, module]) => isIcon(module)) as [
  string,
  Icons.Icon,
][];
...
describe("Custom icons can be rendered", () => {
  ...
  it("Custom SSR icon [regular] renders", () => {
    const result = render(<CustomSSRIcon weight="regular" data-testid="test" />);
    expect(getByTestId(result.container, "test")).toBeTruthy();
  });
  ...
  it("Custom SSR icon [duotone] renders", () => {
    const result = render(<CustomSSRIcon weight="duotone" data-testid="test" />);
    expect(getByTestId(result.container, "test")).toBeTruthy();
  });
});
