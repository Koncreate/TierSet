...
(function main() {
  exec("git submodule update --remote --init --force --recursive", (err, stdout, stderr) => {
    if (err) {
    ...
    console.log(`${chalk.inverse.green(" OK ")} Updated submodule @phosphor-icons/core`);
...
/**
 * ${WEIGHTS.map(
   (weight) => `@${weight} ![img](data:image/svg+xml;base64,${icon[weight].preview})`,
 ).join("\n * ")}
 */`;
...
export const ${name} = I;
export { I as ${name}Icon${!!ALIASES[key] ? `, I as ${pascalize(ALIASES[key])}Icon` : ""} }
`;
