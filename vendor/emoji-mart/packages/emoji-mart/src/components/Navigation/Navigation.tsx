...
export default class Navigation extends PureComponent {
  ...
  renderIcon(category) {
    ...
    if (icon) {
      if (icon.svg) {
        return <span class="flex" dangerouslySetInnerHTML={{ __html: icon.svg }}></span>;
      }
    ...
    const categoryIcons = Icons.categories[category.id] || Icons.categories.custom;

    const style = this.props.icons == "auto" ? THEME_ICONS[this.props.theme] : this.props.icons;
