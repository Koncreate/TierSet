...
function buildData({ set, version } = {}) {
  ...
  emojiData.forEach((datum) => {
    ...
    let name = inflection.titleize(datum.name || datum.short_name.replace(/\-/g, " ") || "");
    ...
    let keywords = ids
      ...
      .flat()
      .filter((word, i, words) => {
        return words.indexOf(word) === i && name.toLowerCase().split(/\s/).indexOf(word) == -1;
      });
    ...
    if (datum.skin_variations) {
      for (let skin of SKINS) {
        let skinDatum = datum.skin_variations[skin] || datum.skin_variations[`${skin}-${skin}`];
  ...
  if (!DRY_RUN) {
    ...
    mkdir(folder, { recursive: true }, () => {
      writeFile(`${folder}/${set || "all"}.json`, JSON.stringify(data), (err) => {
        if (err) throw err;
      });
    });
  }
